...
def filterClient(request,format=None):
    try:
        if request.method == 'POST':
            ...
            obj = obj.exclude(CLIENT_NAME = 'nan')
            obj = obj.values('CLIENT_NAME').annotate(
                                count = Count(F('REVIEW_ID')),
                                client_name = F('CLIENT_NAME')
                              )\
                     .order_by('client_name').order_by('-count')
            obj = list(obj.values_list('client_name',flat=True).distinct())
            obj.insert(0,'Top 5')
            obj.insert(1,'Top 10')
            # obj = list(obj)
            # # obj.sort()
    ...
    # try:
        if request.method == 'POST':
            ...
            if '' not in state:
                ...
                obj = obj.filter(REGION__in = state)
            if '' not in clinic:
            ...
            if '' not in client:
                ...
                top_10 = []
                if 'Top 5' in client:
                    top_5 = obj.values('CLIENT_NAME').annotate(
                                count = Count(F('REVIEW_ID')),
                                client_name = F('CLIENT_NAME')
                              )\
                             .order_by('client_name').order_by('-count')
                    top_5 = list(top_5.values_list('client_name',flat=True).distinct())[:6]
                if 'Top 10' in client:
                    top_10 = obj.values('CLIENT_NAME').annotate(
                                count = Count(F('REVIEW_ID')),
                                client_name = F('CLIENT_NAME')
                              )\
                             .order_by('client_name').order_by('-count')
                    top_10 = list(top_10.values_list('client_name',flat=True).distinct())[:11]
                client = top_5 + top_10 + client  
    ...
    try:
        if request.method == 'POST':
            ...
            if '' not in client:
                ...
                clients = clients.filter(CLIENT_NAME__in = client).values_list('CLIENT_NAME',flat=True).distinct()
